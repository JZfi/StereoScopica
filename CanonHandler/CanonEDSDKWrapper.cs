/*
 * This Canon EDSDK wrapper is based on Johannes Bildstein's LGPL 3.0 licensed code published at:
 * http://www.codeproject.com/Articles/688276/Canon-EDSDK-Tutorial-in-Csharp 
 * 
 * 
 * Modifications
 *  - Class renamed from SDKHandler to CanonEDSDKWrapper
 *  - Fixed string typo in the AV states (value " 8", removed leading space)
 *  - The Error property in the EDSDKWrapper class now tries to find the constant name for the error code using an added method FindConstantName()
 *  - Fixed a bug in the live view loop in DownloadEvf() occuring on camera disconnect during an active live view state
 *    There still seems to exist errorneous situations where an exception is thrown on EDSDK errors (while closing sessions etc)
 */
using System;
...
namespace CanonEDSDKWrapper
{
    public class EDSDKWrapper : IDisposable
    {
        // -------------------------------------------------------------
        // Helper method to retrieve the constant's name by value using reflection (used for EDSDK error values)
        // Modified version, original (C) Jon Skeet, http://stackoverflow.com/a/1064231
        public string FindConstantName<T>(Type containingType, T value)
        {
            EqualityComparer<T> comparer = EqualityComparer<T>.Default;

            foreach (System.Reflection.FieldInfo field in containingType.GetFields
                     (System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public))
            {
                if (field.FieldType == typeof(T) &&
                    comparer.Equals(value, (T)field.GetValue(null)) &&
                    field.Name.Contains("EDS_ERR")) // Match only edsdk error codes
                {
                    return field.Name; // There could be others, of course...
                }
            }
            return "UNKNOWN ERROR";// null; // Or throw an exception
        }
        // -------------------------------------------------------------
        ...
        public uint Error
        {
            ...
            set {
                ...
                {
                    throw new Exception("EDSDK ERROR (" + value + ") " + FindConstantName<uint>(typeof(EDSDK), value));
                }
            }
        }
            ...
            LVThread = new Thread(delegate()
            {
                ...
                while (IsLiveViewOn)
                {
                    lock (LVlock)
                    {
                        ...
                        // Busy state added as it is present in EDSDK cpp sample code along with notready (just retry)
                        if (err == EDSDK.EDS_ERR_OBJECT_NOTREADY || err == EDSDK.EDS_ERR_DEVICE_BUSY) continue;
                        // Halt on any other error (may be a wrong action but this prevents exceptions being thrown or infinte looping on an erroneous state)
                        else if (err != EDSDK.EDS_ERR_OK)
                        {
                            // We jump out of the while on an error and isliveviewon would stay as true otherwise
                            IsLiveViewOn = false;
                            break;
                        }
                        //get pointer
                ...
                //stop the live view
                // Don't try to set it as it will trigger an error on at least EDS_ERR_COMM_DISCONNECTED
